<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ShopItemSearch.aspx.cs" Inherits="ShopItemSearch" %>
...
<script type="text/javascript">
    ...
    function InitItemTitle() {
        ...
        html.push("             <li> <a class='item-href' href='#none'>" + category_name + "</a> </li> ");
        html.push("     </ul> ");
        html.push(" </div> ");
    ...
    function ResultSelectList(data) {
        var html = [];
        var iIndex = 0;
        var jsonValues = jQuery.parseJSON(JSON.stringify(data));
        ...
        while (iIndex < jsonValues.length) {
            ...
            if (category == jsonValue.value) {
                select = "select";
                selectTabItemID = "#tab_" + jsonValue.value;
            }
            html.push(" <li class='tab-item " + select + "' id='tab_" + jsonValue.value + "' onclick='TabItemChange(this, \"" + jsonValue.name + "\", \"" + jsonValue.value + "\");'> <i class='mark'></i>" + jsonValue.name + "</li>");
            iIndex++;
        }
        $('#products_category').html(html.join(''));
    }
    ...
    function ResultShopItem(data) {
        ...
        for (var index = 0; index < jsonValues.length; index++) {
            ...
            html.push("</a>");
            html.push("</div>");
            html.push("<div class='item-data-description' style='height:100px;'>" + jsonValue.description);
            html.push("</div>");
            ...
            html.push("<button class='btn-like fly-shop-item-like' id='like" + index + "'  type='button' nclick='AddLove(\"" + jsonValue.product_id + "\",\"" + jsonValue.category + "\",\"" + jsonValue.category_name + "\",\"" + jsonValue.name + "\",\"" + jsonValue.price + "\",\"1\")'  value=''></button>");
            html.push("</div>");
            html.push("</div>");
            html.push("</div>");
        }
        $('#search_items_div').html(html.join(""));
    }
...
<script type="text/javascript">
    ...
    function TabItemChange(item, name, category) {
        $(this.selectTabItemID).removeClass('select');
        $(item).addClass('select');
        this.selectTabItemID = '#' + item.id;
        var arg = fnGetUpdateQuery("category", category);
        arg = fnUpdateQuery(arg, "category_name", name);
        this.category = category;
        this.category_name = name;
        InitShopItem();
        window.history.pushState('', 'uselessTitle', "ShopItemSearch.aspx?" + arg);
    }
            ...
            <div id="html_item_list" style="margin-top: 80px; margin-bottom: 80px; float: left; width: 100%;">
                <div class="tab-style2">
                    <div class="col-2-left" style="border: 1px solid #dfdfdf; border-radius: 5px;">
                        <ul class="tab-items" id="products_category">
                        </ul>
                    </div>
                    <div class="col-10-right search-item-div" id="search_items_div">
                        <%--                        <div class="col-3">
                            <div class="item-data">
                                <div class="item-data-img">
                                    <a href="123">
                                        <img class="item-img-1" src="ShopItem/002/Pokemon_122.png" style="max-width: 100%; max-height: 100%;" />
                                    </a>
                                </div>
                                <hr style="border: none; background-color: #a3a3a3; height: 1px;" />
                                <div class="item-data-name">
                                    <a href="123">123
                                    </a>
                                </div>
                                <div class="item-data-content">
                                    asdasdasdasdasdasdasasdsasdasdasdasdasdasdasasdasdasdasdasdasdasasdasdasdasdasdasdasasdasdasdasdasdasdasasdasdasdasdasdasdasasdasdasdasdasdasdas
                                </div>
                                <div class="item-data-content">
                                    NT$ 1000
                                </div>
                                <div style="width: 100%;">
                                    <button class="btn-buy fly-shop-item-buy" id="buy1" type='button' value=""></button>
                                    <button class="btn-like fly-shop-item-like" id="like1" type='button' value=""></button>
                                </div>
                            </div>
                        </div>--%>
                    </div>
