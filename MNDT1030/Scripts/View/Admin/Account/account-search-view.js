...
function AccountSearchView(accountSearchPresenter) {
    this.g_accountSearchPresenter = accountSearchPresenter;
...
AccountSearchView.prototype = {

    TAG: 'AccountSearchView',
    ...
    fnChangeUrl: function (arg) {
        ...
        setTimeout(function () {
            that.g_accountSearchPresenter.fnInitList();
        }, 300);
    },
    ...
    fnPushColumns: function (jsonValue) {
        ...
        this.g_arrayHtml.push("      <td class='key'>" + jsonValue.id + "</td> ");
        ...
        this.g_arrayHtml.push("               <button type='button' class='btn-img-style1' onclick='accountSearchView.fnOpenEdit(\"" + jsonValue.id + "\", \"" + jsonValue.NUM + "\")' ><img src='Image/edit.png' /></button> ");
        this.g_arrayHtml.push("               <button type='button' class='btn-img-style2' onclick='accountSearchView.fnDetele(\"" + jsonValue.id + "\")'><img src='Image/delete.png' /></button> ");
        this.g_arrayHtml.push("          </div> ");
    ...
    fnPushPageNumber: function (name, page, select) {
        this.g_arrayHtml.push("  <li><a class='" + select + "' onclick='accountSearchView.fnPageNumberChange(\"" + page + "\")' >" + name + "</a></li> ");
    },
    ...
    fnImport: function () {
        ...
        if (files.length > 0) {
            ...
            this.g_accountSearchPresenter.fnImport(formData);
        }
    },

    fnMaxPageSizeChange: function (item) {
        this.g_accountSearchPresenter.fnMaxPageSizeChange(item.value);
    },

    fnOrderChange: function (value) {
        this.g_accountSearchPresenter.fnOrderChange(value);
    },

    fnPageNumberChange: function (value) {
        this.g_accountSearchPresenter.fnPageNumberChange(value);
    },
    ...
    fnFilterChange: function () {
        ...
        this.g_accountSearchPresenter.fnFilterChange(filter);
    },
    ...
    fnOpenEdit: function (id, NUM) {
        var filter = window.location.search.substring(1);
        filter = (filter.length > 0) ? '&' + filter : filter;
        window.location = 'AccountEditView.aspx?id=' + id + '&NUM=' + NUM + filter;
    },
    ...
    fnDeteles: function () {
        if (confirm("確定要刪除嗎!")) {
            ...
            accountSearchPresenter.fnDeletes(sIds);
        }
    },

    fnDetele: function (key) {
        if (confirm("確定要刪除嗎!")) {
            accountSearchPresenter.fnDelete(key);
        }
    }
};
