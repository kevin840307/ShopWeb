...
function CompanySearchView(companySearchPresenter) {
    this.g_companySearchPresenter = companySearchPresenter;
...
CompanySearchView.prototype = {

    TAG: 'CompanySearchView',
    ...
    fnChangeUrl: function (arg) {
        ...
        setTimeout(function () {
            that.g_companySearchPresenter.fnInitList();
        }, 300);
    },
    ...
    fnPushColumns: function (jsonValue) {
        ...
        this.g_arrayHtml.push("      <td name='company_id'>" + jsonValue.company_id + "</td> ");
        ...
        this.g_arrayHtml.push("               <button type='button' class='btn-img-style1' onclick='companySearchView.fnOpenEdit(\"" + jsonValue.company_id + "\", \"" + jsonValue.NUM + "\")' ><img src='Image/edit.png' /></button> ");
        this.g_arrayHtml.push("               <button type='button' class='btn-img-style2' onclick='companySearchView.fnDetele(\"" + jsonValue.company_id + "\")'><img src='Image/delete.png' /></button> ");
        this.g_arrayHtml.push("          </div> ");
    ...
    fnPushPageNumber: function (name, page, select) {
        this.g_arrayHtml.push("  <li><a class='" + select + "' onclick='companySearchView.fnPageNumberChange(\"" + page + "\")' >" + name + "</a></li> ");
    },
    ...
    fnImport: function () {
        ...
        if (files.length > 0) {
            ...
            this.g_companySearchPresenter.fnImport(formData);
        }
    },

    fnMaxPageSizeChange: function (item) {
        this.g_companySearchPresenter.fnMaxPageSizeChange(item.value);
    },

    fnOrderChange: function (value) {
        this.g_companySearchPresenter.fnOrderChange(value);
    },

    fnPageNumberChange: function (value) {
        this.g_companySearchPresenter.fnPageNumberChange(value);
    },
    ...
    fnFilterChange: function () {
        ...
        this.g_companySearchPresenter.fnFilterChange(filter);
    },
    ...
    fnOpenEdit: function (company_id, NUM) {
        var filter = window.location.search.substring(1);
        filter = (filter.length > 0) ? '&' + filter : filter;
        window.location = 'CompanyEditView.aspx?company_id=' + company_id + '&NUM=' + NUM + filter;
    },
    ...
    fnDeteles: function () {
        if (confirm("確定要刪除嗎!")) {
            ...
            $(this.g_sListID + ' > tbody > tr').each(function () {
                ...
                if (sCheck == 'checked') {
                    sCompanyIds.push("'" + $(this).find(' > td[name="company_id"]').text() + "'");
                }
            });
            companySearchPresenter.fnDeletes(sCompanyIds);
        }
    },

    fnDetele: function (sCompanyId) {
        if (confirm("確定要刪除嗎!")) {
            companySearchPresenter.fnDelete(sCompanyId);
        }
    }
};
