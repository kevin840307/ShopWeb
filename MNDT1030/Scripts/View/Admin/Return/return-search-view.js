function ReturnSearchView(returnSearchPresenter) {
    this.g_returnSearchPresenter = returnSearchPresenter;
    this.g_sListID = '#list';
...
ReturnSearchView.prototype = {
    TAG: 'ReturnSearchView',
    ...
    fnChangeUrl: function (arg) {
        ...
        setTimeout(function () {
            that.g_returnSearchPresenter.fnInitList();
        }, 300);
    },
    ...
    fnPushColumns: function (jsonValue) {
        ...
        this.g_arrayHtml.push("               <button type='button' class='btn-img-style1' onclick='returnSearchView.fnOpenEdit(\"" + jsonValue.return_id + "\", \"" + jsonValue.NUM + "\")' ><img src='Image/edit.png' /></button> ");
        this.g_arrayHtml.push("               <button type='button' class='btn-img-style2' onclick='returnSearchView.fnDetele(\"" + jsonValue.return_id + "\")'><img src='Image/delete.png' /></button> ");
        this.g_arrayHtml.push("          </div> ");
    ...
    fnPushPageNumber: function (name, page, select) {
        this.g_arrayHtml.push("  <li><a class='" + select + "' onclick='returnSearchView.fnPageNumberChange(\"" + page + "\")' >" + name + "</a></li> ");
    },
    ...
    fnFilterChange: function () {
        ...
        this.g_returnSearchPresenter.fnFilterChange(filter);
    },

    fnMaxPageSizeChange: function (item) {
        this.g_returnSearchPresenter.fnMaxPageSizeChange(item.value);
    },

    fnOrderChange: function (value) {
        this.g_returnSearchPresenter.fnOrderChange(value);
    },

    fnPageNumberChange: function (value) {
        this.g_returnSearchPresenter.fnPageNumberChange(value);
    },

    fnDetele: function (id) {
        if (confirm("確定要刪除嗎!")) {
            returnSearchPresenter.fnDelete(id);
        }
    },
    ...
    fnOpenEdit: function (return_id, NUM) {
        ...
        window.location = 'ReturnEditView.aspx?return_id=' + return_id + '&NUM=' + NUM + filter;
    },
