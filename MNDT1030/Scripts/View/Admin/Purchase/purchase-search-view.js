function PurchaseSearchView(purchaseSearchPresenter) {
    this.g_purchaseSearchPresenter = purchaseSearchPresenter;
    this.g_sListID = '#list';
...
PurchaseSearchView.prototype = {
    TAG: 'PurchaseSearchView',
    ...
    fnChangeUrl: function (arg) {
        ...
        setTimeout(function () {
            that.g_purchaseSearchPresenter.fnInitList();
        }, 300);
    },
    ...
    fnPushColumns: function (jsonValue) {
        ...
        this.g_arrayHtml.push("               <button type='button' class='btn-img-style1' onclick='purchaseSearchView.fnOpenEdit(\"" + jsonValue.order_id + "\", \"" + jsonValue.NUM + "\")' ><img src='Image/edit.png' /></button> ");
        this.g_arrayHtml.push("               <button type='button' class='btn-img-style2' onclick='purchaseSearchView.fnDetele(\"" + jsonValue.order_id + "\")'><img src='Image/delete.png' /></button> ");
        this.g_arrayHtml.push("          </div> ");
    ...
    fnPushPageNumber: function (name, page, select) {
        this.g_arrayHtml.push("  <li><a class='" + select + "' onclick='purchaseSearchView.fnPageNumberChange(\"" + page + "\")' >" + name + "</a></li> ");
    },
    ...
    fnFilterChange: function () {
        ...
        this.g_purchaseSearchPresenter.fnFilterChange(filter);
    },

    fnMaxPageSizeChange: function (item) {
        this.g_purchaseSearchPresenter.fnMaxPageSizeChange(item.value);
    },

    fnOrderChange: function (value) {
        this.g_purchaseSearchPresenter.fnOrderChange(value);
    },

    fnPageNumberChange: function (value) {
        this.g_purchaseSearchPresenter.fnPageNumberChange(value);
    },

    fnDetele: function (id) {
        if (confirm("確定要刪除嗎!")) {
            purchaseSearchPresenter.fnDelete(id);
        }
    },
    ...
    fnOpenEdit: function (order_id, NUM) {
        ...
        window.location = 'PurchaseEditView.aspx?order_id=' + order_id + '&NUM=' + NUM + filter;
    },
