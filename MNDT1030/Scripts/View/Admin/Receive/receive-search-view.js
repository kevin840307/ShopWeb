function ReceiveSearchView(receiveSearchPresenter) {
    this.g_receiveSearchPresenter = receiveSearchPresenter;
    this.g_sListID = '#list';
...
ReceiveSearchView.prototype = {
    TAG: 'ReceiveSearchView',
    ...
    fnChangeUrl: function (arg) {
        ...
        setTimeout(function () {
            that.g_receiveSearchPresenter.fnInitList();
        }, 300);
    },
    ...
    fnPushColumns: function (jsonValue) {
        ...
        this.g_arrayHtml.push("               <button type='button' class='btn-img-style1' onclick='receiveSearchView.fnOpenEdit(\"" + jsonValue.order_id + "\", \"" + jsonValue.NUM + "\")' ><img src='Image/edit.png' /></button> ");
        this.g_arrayHtml.push("               <button type='button' class='btn-img-style2' onclick='receiveSearchView.fnDetele(\"" + jsonValue.order_id + "\")'><img src='Image/delete.png' /></button> ");
        this.g_arrayHtml.push("          </div> ");
    ...
    fnPushPageNumber: function (name, page, select) {
        this.g_arrayHtml.push("  <li><a class='" + select + "' onclick='receiveSearchView.fnPageNumberChange(\"" + page + "\")' >" + name + "</a></li> ");
    },
    ...
    fnFilterChange: function () {
        ...
        this.g_receiveSearchPresenter.fnFilterChange(filter);
    },

    fnMaxPageSizeChange: function (item) {
        this.g_receiveSearchPresenter.fnMaxPageSizeChange(item.value);
    },

    fnOrderChange: function (value) {
        this.g_receiveSearchPresenter.fnOrderChange(value);
    },

    fnPageNumberChange: function (value) {
        this.g_receiveSearchPresenter.fnPageNumberChange(value);
    },

    fnDetele: function (id) {
        if (confirm("確定要刪除嗎!")) {
            receiveSearchPresenter.fnDelete(id);
        }
    },
    ...
    fnOpenEdit: function (order_id, NUM) {
        ...
        window.location = 'ReceiveEditView.aspx?order_id=' + order_id + '&NUM=' + NUM + filter;
    },
