...
function SalesSearchPresenter(salesSearchView) {
    this.g_salesSearchView = salesSearchView;
    this.g_iPage = 1;
    this.g_iMaxPageSize = 8;
}

SalesSearchPresenter.prototype = {
    TAG: 'SalesSearchPresenter',

    fnInitialization: function () {
        this.g_salesSearchView.fnInitialization();
        this.fnInitList();
    },

    fnPageNumberChange: function (page) {
        var sArg = '?' + fnGetUpdateQuery('page', page);
        this.g_salesSearchView.fnChangeUrl(sArg);
    },
    ...
    fnInitList: function () {
        ...
        this.g_order = order;
        this.g_iPage = iPage;
        this.g_salesSearchView.fnShowFilterView(this.filter);
        this.fnSelects();
    },
    ...
    fnResultSelect: function (data) {
        ...
        while (iIndex < jsonValues.length) {
            ...
            this.g_salesSearchView.fnPushColumns(jsonValue);
            iIndex++;
        }
        this.g_salesSearchView.fnShowList();
        if (jsonValues.length == 0) {
            this.g_salesSearchView.fnShowEmptyColumns();
        }
        this.fnCount();
    },
    ...
    fnResultCount: function (data) {
        ...
        if (this.g_iPage != 1) {
            this.g_salesSearchView.fnPushPageNumber('|<<', 1, '');
        }

        var iLastPage = this.g_iPage - 2; // 若是第一頁則不顯示
        if (iLastPage > 0) {
            this.g_salesSearchView.fnPushPageNumber('<<', iLastPage, '');
        }

        var iCount = 1;
        var iMinPage = (this.g_iPage - 5) < 2 ? 1 : (this.g_iPage - 5); // 最小為1頁
        for (var iIndex = iMinPage ; iIndex < this.g_iPage; iIndex++) {
            this.g_salesSearchView.fnPushPageNumber(iIndex, iIndex, '');
            iCount++;
        }

        this.g_salesSearchView.fnPushPageNumber(this.g_iPage, this.g_iPage, 'select');
        ...
        if (iNextPage < iPage) {
            this.g_salesSearchView.fnPushPageNumber('>>', iNextPage, '');
        }

        if (this.g_iPage != iPage) {
            this.g_salesSearchView.fnPushPageNumber('>>|', iPage, '');
        }
        this.g_salesSearchView.fnShowPageNumber();
        this.g_salesSearchView.fnInitListEvent();
    },
    ...
    fnResultDelete: function (data) {
        ...
        if (jsonValue.msg == 'Y') {
            ...
            this.g_salesSearchView.fnGeneralMessage("刪除成功");
        } else {
            this.g_salesSearchView.fnErrorMessage(jsonValue.msg);
        }
    }
}
