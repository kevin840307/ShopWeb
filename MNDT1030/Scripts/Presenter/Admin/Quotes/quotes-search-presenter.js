...
function QuotesSearchPresenter(quotesSearchView) {
    this.g_quotesSearchView = quotesSearchView;
    this.g_iPage = 1;
    this.g_iMaxPageSize = 8;
}

QuotesSearchPresenter.prototype = {
    TAG: 'QuotesSearchPresenter',

    fnInitialization: function () {
        this.g_quotesSearchView.fnInitialization();
        this.fnInitList();
    },

    fnPageNumberChange: function (page) {
        var sArg = '?' + fnGetUpdateQuery('page', page);
        this.g_quotesSearchView.fnChangeUrl(sArg);
    },
    ...
    fnInitList: function () {
        ...
        this.g_order = order;
        this.g_iPage = iPage;
        this.g_quotesSearchView.fnShowFilterView(this.filter);
        this.fnSelects();
    },
    ...
    fnResultSelect: function (data) {
        ...
        while (iIndex < jsonValues.length) {
            ...
            this.g_quotesSearchView.fnPushColumns(jsonValue);
            iIndex++;
        }
        this.g_quotesSearchView.fnShowList();
        if (jsonValues.length == 0) {
            this.g_quotesSearchView.fnShowEmptyColumns();
        }
        this.fnCount();
    },
    ...
    fnResultCount: function (data) {
        ...
        if (this.g_iPage != 1) {
            this.g_quotesSearchView.fnPushPageNumber('|<<', 1, '');
        }

        var iLastPage = this.g_iPage - 2; // 若是第一頁則不顯示
        if (iLastPage > 0) {
            this.g_quotesSearchView.fnPushPageNumber('<<', iLastPage, '');
        }

        var iCount = 1;
        var iMinPage = (this.g_iPage - 5) < 2 ? 1 : (this.g_iPage - 5); // 最小為1頁
        for (var iIndex = iMinPage ; iIndex < this.g_iPage; iIndex++) {
            this.g_quotesSearchView.fnPushPageNumber(iIndex, iIndex, '');
            iCount++;
        }

        this.g_quotesSearchView.fnPushPageNumber(this.g_iPage, this.g_iPage, 'select');
        ...
        if (iNextPage < iPage) {
            this.g_quotesSearchView.fnPushPageNumber('>>', iNextPage, '');
        }

        if (this.g_iPage != iPage) {
            this.g_quotesSearchView.fnPushPageNumber('>>|', iPage, '');
        }
        this.g_quotesSearchView.fnShowPageNumber();
        this.g_quotesSearchView.fnInitListEvent();
    },
    ...
    fnResultDelete: function (data) {
        ...
        if (jsonValue.msg == 'Y') {
            ...
            this.g_quotesSearchView.fnGeneralMessage("刪除成功");
        } else {
            this.g_quotesSearchView.fnErrorMessage(jsonValue.msg);
        }
    }
}
