...
function SReturnSearchPresenter(sreturnSearchView) {
    this.g_sreturnSearchView = sreturnSearchView;
    this.g_iPage = 1;
    this.g_iMaxPageSize = 8;
}

SReturnSearchPresenter.prototype = {
    TAG: 'SReturnSearchPresenter',

    fnInitialization: function () {
        this.g_sreturnSearchView.fnInitialization();
        this.fnInitList();
    },

    fnPageNumberChange: function (page) {
        var sArg = '?' + fnGetUpdateQuery('page', page);
        this.g_sreturnSearchView.fnChangeUrl(sArg);
    },
    ...
    fnInitList: function () {
        ...
        this.g_order = order;
        this.g_iPage = iPage;
        this.g_sreturnSearchView.fnShowFilterView(this.filter);
        this.fnSelects();
    },
    ...
    fnResultSelect: function (data) {
        ...
        while (iIndex < jsonValues.length) {
            ...
            this.g_sreturnSearchView.fnPushColumns(jsonValue);
            iIndex++;
        }
        this.g_sreturnSearchView.fnShowList();
        if (jsonValues.length == 0) {
            this.g_sreturnSearchView.fnShowEmptyColumns();
        }
        this.fnCount();
    },
    ...
    fnResultCount: function (data) {
        ...
        if (this.g_iPage != 1) {
            this.g_sreturnSearchView.fnPushPageNumber('|<<', 1, '');
        }

        var iLastPage = this.g_iPage - 2; // 若是第一頁則不顯示
        if (iLastPage > 0) {
            this.g_sreturnSearchView.fnPushPageNumber('<<', iLastPage, '');
        }

        var iCount = 1;
        var iMinPage = (this.g_iPage - 5) < 2 ? 1 : (this.g_iPage - 5); // 最小為1頁
        for (var iIndex = iMinPage ; iIndex < this.g_iPage; iIndex++) {
            this.g_sreturnSearchView.fnPushPageNumber(iIndex, iIndex, '');
            iCount++;
        }

        this.g_sreturnSearchView.fnPushPageNumber(this.g_iPage, this.g_iPage, 'select');
        ...
        if (iNextPage < iPage) {
            this.g_sreturnSearchView.fnPushPageNumber('>>', iNextPage, '');
        }

        if (this.g_iPage != iPage) {
            this.g_sreturnSearchView.fnPushPageNumber('>>|', iPage, '');
        }
        this.g_sreturnSearchView.fnShowPageNumber();
        this.g_sreturnSearchView.fnInitListEvent();
    },
    ...
    fnResultDelete: function (data) {
        ...
        if (jsonValue.msg == 'Y') {
            ...
            this.g_sreturnSearchView.fnGeneralMessage("刪除成功");
        } else {
            this.g_sreturnSearchView.fnErrorMessage(jsonValue.msg);
        }
    }
}
