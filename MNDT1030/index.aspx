...
<script type="text/javascript">
    ...
    function ResultHotShopItem(data) {
        let html = [];
        html.push(" <h3 class='text-style1'>推薦商品</h3> ");
        var index = 0;
        var jsonValues = jQuery.parseJSON(JSON.stringify(data));
        while (index < jsonValues.length) {
            var jsonValue = $.parseJSON(JSON.stringify(jsonValues[index]));
            var href = "ShopItemContent.aspx?product_id=" + jsonValue.product_id + "&category=" + jsonValue.category + "&category_name=" + jsonValue.category_name + "&name=" + jsonValue.name;
            html.push(" <div class='hot-item-div'> ");
            html.push("      <div class='hot-img-div'> ");
            html.push("          <a href='" + href + "'> <img class='hot-item-img hot-item-img-" + index + "' src='ShopItem/" + jsonValue.product_id + "/first.png' title='" + jsonValue.name + "' ></a> ");
            html.push("      </div> ");
            html.push("      <div class='hot-content-div'> ");
            html.push("          <a href='" + href + "'>" + jsonValue.name + "</a>");
            html.push("          <p class='hot-description'>" + jsonValue.description + "</p>");
            html.push("          <p class='hot-price'>NT$" + jsonValue.price + "</p>");
            html.push("      </div> ");
            html.push("      <div class='hot-content-button'> ");
            html.push("          <button class='btn-buy fly-shop-item-buy' id='buy" + index + "' onclick='AddCart(\"" + jsonValue.product_id + "\",\"" + jsonValue.category + "\",\"" + jsonValue.category_name + "\",\"" + jsonValue.name + "\",\"" + jsonValue.price + "\",\"1\")' type='button' title='放入購物車'> ");
            html.push("          </button>");
            html.push("          <button class='btn-like fly-shop-item-like' id='like" + index + "' onclick='AddLove(\"" + jsonValue.product_id + "\",\"" + jsonValue.category + "\",\"" + jsonValue.category_name + "\",\"" + jsonValue.name + "\",\"" + jsonValue.price + "\",\"1\")' type='button' title='我的最愛'> ");
            html.push("          </button>");
            html.push("      </div> ");
            html.push(" </div> ");
            index++;
