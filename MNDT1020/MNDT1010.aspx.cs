...
public partial class MNDT1010 : System.Web.UI.Page
{
    private void fnSearchData()
    {
        string sSql = "  SELECT  [account_id]  " +
                        ...
                        "    FROM [MNDTaccount]  " +
                        "    WHERE 1 = 1 ";
        Functions.fnAddCondition(ref text_account_id, ref sSql, "account_id");
        Functions.fnAddCondition(ref text_account_name, ref sSql, "account_name");
    ...
    private void fnInit()
    {
        ...
        ((Label)Master.FindControl("lab_title")).Text = "使用者資料維護<MNDT1010>";
    ...
    protected void grid_data_m_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        switch (sCommand)
        {
            ...
            case "SelectData":
                ...
                string sAccountId = ((Label)grid_data_m.Rows[fnGetGridViewSelect(iIndex)].FindControl("lab_account_id")).Text.ToString();
                fnOpenEdit(sAccountId);
    ...
    private void fnSetSelectMasterView(string sAccountId)
    {
        string sMasterSql = "  SELECT  [account_id]  " +
                       "        ,[account_password]  " +
                       "        ,[account_name]  " +
                       "        ,[account_addr]  " +
                       "        ,[account_phone]  " +
    ...
    {
        string sSql = "  INSERT INTO [MNDTaccount]  " +
                        "             ([account_id]  " +
                        "             ,[account_password]  " +
                        "             ,[account_name]  " +
                        "             ,[account_addr]  " +
    ...
    {
        sds_main.InsertParameters["account_id"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_account_id")).Text;
        sds_main.InsertParameters["account_password"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_account_password")).Text;
        sds_main.InsertParameters["account_name"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_account_name")).Text;
        sds_main.InsertParameters["account_addr"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_account_addr")).Text;
        sds_main.InsertParameters["account_phone"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_account_phone")).Text;
        sds_main.InsertParameters["account_department"].DefaultValue = ((DropDownList)fv_master_form.FindControl("drop_account_department")).SelectedValue;
        sds_main.Insert();
    }
    ...
    protected void sds_main_Inserted(object sender, SqlDataSourceStatusEventArgs e)
    {
        ...
        if (e.Exception == null)
        {
            string sAccountId = ((TextBox)fv_master_form.FindControl("text_account_id")).Text;
            fnOpenEdit(sAccountId);
        }
    }
    ...
    protected void drop_account_department_DataBinding(object sender, EventArgs e)
    {
