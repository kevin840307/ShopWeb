...
public partial class MNDT3020 : System.Web.UI.Page
{
    private void fnSearchData()
    {
        string sMasterSql = "  SELECT [purchase_m].[purchase_order]  " +
                            "        ,[purchase_m].[purchase_id]  " +
                            "        ,[account].[account_name] " +
                            "        ,CONVERT(char, [purchase_m].[purchase_datetime], 111) 'purchase_datetime'  " +
                            "        ,CONVERT(char, [purchase_m].[purchase_to_datetime], 111) 'purchase_to_datetime'  " +
                            "        ,CASE [purchase_m].[purchase_complete] WHEN '1' THEN '已採購' ELSE '採購中' END 'purchase_complete'  " +
                            "        ,[purchase_m].[purchase_remarks]  " +
                            "  FROM [MNDTpurchase_master] [purchase_m] LEFT JOIN [MNDTaccount] [account] " +
                            "  ON [purchase_m].[purchase_id] = [account].[account_id] " +
                            "  WHERE 1 = 1 ";
        Functions.fnAddCondition(ref text_purchase_order, ref sMasterSql, "[purchase_m].[purchase_order]");
        Functions.fnAddCondition(ref text_purchase_datetime, ref sMasterSql, "[purchase_m].[purchase_datetime]");
        Functions.fnAddCondition(ref text_purchase_to_datetime, ref sMasterSql, "[purchase_m].[purchase_to_datetime]");
        Functions.fnAddCondition(ref drop_purchase_complete, ref sMasterSql, "[purchase_m].[purchase_complete]");
        Functions.fnAddCondition(ref drop_purchase_id, ref sMasterSql, "[purchase_m].[purchase_id]");
    ...
    private void fnInit()
    {
        ...
        ((Label)Master.FindControl("lab_title")).Text = "採購資料維護<MNDT3020>";
    ...
    {
        string sSql = "  UPDATE [MNDTpurchase_master]  " +
                    "     SET [purchase_complete] = '" + sStatus + "' " +
                    "  WHERE [purchase_order] = '" + sOrder + "' ";
        if (Functions.fnExecuteSQL(sSql, "MNDT") == "")
        {
            fnInsertReceiveM(sOrder);
        }
        else
        {
    ...
    private void fnInsertReceiveM(string sOrder)
    {
    ...
    private void fnInsertReceiveD(string sOrder)
    {
        ...
        if (Functions.fnExecuteSQL(sSql, "MNDT") == "")
        {
            fnInsertStock(sOrder);
        }
        else
        {
            Functions.fnShowMessage(this, Functions.fnExecuteSQL(sSql, "MNDT"), "進貨明細新增失敗");
        }
    }

    private void fnInsertStock(string sOrder)
    {
        string sSql = "  INSERT INTO [MNDTstock]  " +
                        ...
                        "  FROM [MNDTpurchase_details]  " +
                        "  WHERE [purchase_order] = '" + sOrder + "'  ";
        Functions.fnShowMessage(this, Functions.fnExecuteSQL(sSql, "MNDT"), "採購成功");
    }
    ...
    private void fnSetSelectMasterView(string sOrder)
    {
        string sMasterSql = "  SELECT TOP 1 [purchase_order]  " +
                           "        ,[purchase_id]  " +
                           "        ,CONVERT(char, [purchase_datetime], 111) 'purchase_datetime'  " +
                           "        ,CONVERT(char, [purchase_to_datetime], 111) 'purchase_to_datetime'  " +
                           "        ,CASE [purchase_complete] WHEN '1' THEN '已採購' ELSE '採購中' END 'purchase_complete'  " +
                           "        ,[purchase_remarks]  " +
                           "  FROM [MNDTpurchase_master]  " +
                           "  WHERE [purchase_order] = '" + sOrder + "' ";
        sds_main.SelectCommand = sMasterSql;
    ...
    private void fnInsertMaster()
    {
        ...
        sds_main.InsertParameters["purchase_id"].DefaultValue = ((DropDownList)fv_master_form.FindControl("drop_insert_purchase_id")).SelectedValue;
        sds_main.InsertParameters["purchase_datetime"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_insert_purchase_datetime")).Text;
        sds_main.InsertParameters["purchase_to_datetime"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_insert_purchase_to_datetime")).Text;
        sds_main.InsertParameters["purchase_remarks"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_insert_purchase_remarks")).Text;
        sds_main.Insert();
    }
    ...
    private void fnInsert()
    {
        ...
        sds_detail.InsertParameters["purchase_amount"].DefaultValue = ((TextBox)ctControl.FindControl("text_purchase_amount")).Text;
        sds_detail.InsertParameters["purchase_money"].DefaultValue = ((TextBox)ctControl.FindControl("text_purchase_money")).Text;
        sds_detail.InsertParameters["purchase_remarks"].DefaultValue = ((TextBox)ctControl.FindControl("text_purchase_remarks")).Text;
        sds_detail.Insert();
    }
    ...
    protected void text_purchase_amount_TextChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            TextBox textMoney = ((TextBox)textData.Parent.FindControl("text_purchase_money"));
            textMoney.Text = (iAmount * iPrice).ToString("f3");
        }
        catch
        { }
    }
    protected void drop_purchase_id_DataBinding(object sender, EventArgs e)
    {
