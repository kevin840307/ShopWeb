...
public partial class MNDT2010 : System.Web.UI.Page
{
    private void fnSearchData()
    {
        string sSql = "  SELECT  [client_id]  " +
                        ...
                        "    FROM [MNDTclient]  " +
                        "    WHERE 1 = 1 ";
        Functions.fnAddCondition(ref text_client_id, ref sSql, "client_id");
        Functions.fnAddCondition(ref text_client_name, ref sSql, "client_name");
    ...
    private void fnInit()
    {
        ...
        ((Label)Master.FindControl("lab_title")).Text = "客戶資料維護<MNDT2010>";
    ...
    protected void grid_data_m_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        switch (sCommand)
        {
            ...
            case "SelectData":
                ...
                string sclientId = ((Label)grid_data_m.Rows[fnGetGridViewSelect(iIndex)].FindControl("lab_client_id")).Text.ToString();
                fnOpenEdit(sclientId);
    ...
    private void fnOpenEdit(string sClientId)
    {
        multi_view.ActiveViewIndex = 1;
        fnSetSelectMasterView(sClientId);
        fv_master_form.ChangeMode(FormViewMode.Edit);
    }

    private void fnSetSelectMasterView(string sClientId)
    {
        string sMasterSql = "  SELECT  [client_id]  " +
                       "        ,[client_remarks]  " +
                       "        ,[client_name]  " +
                       "        ,[client_addr]  " +
                       "        ,[client_phone]  " +
    ...
    {
        Session["PrintDT"] = Functions.fnGetDt("SELECT * FROM [MNDTclient]", "MNDT");
    ...
    {
        sds_main.InsertParameters["client_id"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_client_id")).Text;
        sds_main.InsertParameters["client_remarks"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_client_remarks")).Text;
        sds_main.InsertParameters["client_name"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_client_name")).Text;
        sds_main.InsertParameters["client_addr"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_client_addr")).Text;
        sds_main.InsertParameters["client_phone"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_client_phone")).Text;
        sds_main.InsertParameters["client_contact_name"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_client_contact_name")).Text;
        sds_main.InsertParameters["client_contact_phone"].DefaultValue = ((TextBox)fv_master_form.FindControl("text_client_contact_phone")).Text;
        sds_main.Insert();
    }
    ...
    protected void sds_main_Inserted(object sender, SqlDataSourceStatusEventArgs e)
    {
        ...
        if(e.Exception == null)
        {
            string sclientId = ((TextBox)fv_master_form.FindControl("text_client_id")).Text;
            fnOpenEdit(sclientId);
        }
    }
